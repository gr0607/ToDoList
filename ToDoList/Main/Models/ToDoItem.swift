...
extension ToDoItem {
    ...
    static func parse(json: Any) -> [ToDoItem]? {
        ...
        guard let whatString = String(data: data, encoding: .utf8)
                else { return nil }

        let arrayString = whatString.split(separator: "\n")
        var result = [ToDoItem]()
        for arStr in arrayString {

            let data = arStr.data(using: .utf8)!
            do {
                ...
                result.append(ToDoItem(id: id, text: text, priority: priority ?? .meduim, deadLine: myDate))
            } catch {
            }
